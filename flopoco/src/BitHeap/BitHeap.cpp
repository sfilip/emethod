...
namespace flopoco
{
	...
	void BitHeap::addUnsignedBitVector(int weight, string x, unsigned size, int msb, int lsb, bool negativeWeight)
	{
		...
		if((unsigned int)(msb-lsb+1)>size)
	...
	void BitHeap::generatePossibleCompressors()
	{
		...
		{// test
			...
			newVect.push_back(col0);
			newVect.push_back(col1);
			compressor = new BasicCompressor(op->getTarget(), newVect);
			possibleCompressors.push_back(compressor);
		}
		*/
		{
			col0=6; col1=0;
	...
	void BitHeap::generateCompressorVHDL()
	{
		...
		else
		{
			...
			else
			{
				...
				while((int)getMaxHeight() >= op->getTarget()->lutInputs())
				{
		...
		for(unsigned int i=0; i<(sizeof(usedCompressors)/sizeof(bool)); i++)
		{
		...
		if(halfAdder) // but protect, because if the multipliers fits a DSP it is not true
			op->addToGlobalOpList(halfAdder);
		if(fullAdder)
	...
	void BitHeap::applyAdder(int lsb, int msb, bool hasCin)
	{
		...
		OperatorPtr adder = new IntAdder(op->getTarget(), msb-lsb+2);
			...
			*/
			...
			}else
			{
				...
				for(unsigned int i=0; i<(maxWeight-minWeight)/subAddSize; i++)
				{
					...
					}else if((index == (maxWeight-minWeight+2)/subAddSize - 1) && !(((int)(maxWeight-minWeight+2)%subAddSize > 0) && ((int)(maxWeight-minWeight+2)>subAddSize)))
					{
