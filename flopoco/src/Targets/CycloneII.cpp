/*
  A model of the Cyclone II FPGA 
...
namespace flopoco{
	...
	double CycloneII::adderDelay(int size) {
		int subAdd = 0;
	...
	void CycloneII::getAdderParameters(double &k1, double &k2, int size)
	{
		int subAdd = 0;
	...
	double CycloneII::eqComparatorDelay(int size){
		return adderDelay(size) + elemWireDelay_ + lut2_; // TODO: check correctness
	}
	
	double CycloneII::eqConstComparatorDelay(int size){
		return adderDelay(size/2) + elemWireDelay_ + lut2_; // // TODO: check correctness
	}
	
	double CycloneII::carryPropagateDelay() {
		return  fastcarryDelay_; 
	};
	
	double CycloneII::localWireDelay(int fanout){
		return elemWireDelay_;
	};
	
	double CycloneII::distantWireDelay(int n){
		return n*elemWireDelay_;
	};
	
	double CycloneII::lutDelay(){
		return lutDelay_;
	};
	
	double CycloneII::ffDelay(){
		return ffDelay_;
	};
	
	long CycloneII::sizeOfMemoryBlock()
	{
		return sizeOfBlock_;	
	};
	
	DSP* CycloneII::createDSP() 
	{
		int multW, multH;
	...
	bool CycloneII::suggestSubmultSize(int &x, int &y, int wInX, int wInY){
	...
	bool CycloneII::suggestSubaddSize(int &x, int wIn){
	...
	bool  CycloneII::suggestSlackSubaddSize(int &x, int wIn, double slack)
	{
	...
	// TODO: - redo
	int CycloneII::getIntMultiplierCost(int wInX, int wInY)
	{
		int cost = 0;
	...
	//		check validity
	void CycloneII::getDSPWidths(int &x, int &y, bool sign)
	{ 
	...
	int CycloneII::getEquivalenceSliceDSP(){
		int lutCost = 0;
		int x, y;
	...
	int CycloneII::getIntNAdderCost(int wIn, int n)
	{
	...
	void CycloneII::delayForDSP(MultiplierBlock* multBlock, double currentCp, int& cycleDelay, double& cpDelay)
	{
